...
def train(model_path):
    ...
    for e in range(hp.train.epochs):
        ...
        for batch_id, mel_db_batch in enumerate(train_loader):
            ...
            if (batch_id + 1) % hp.train.log_interval == 0:
                mesg = "Epoch:{}[{}/{}],Iteration:{}\tLoss:{:.4f}\tTLoss:{:.4f}\t\n".format(
                    e + 1, batch_id + 1, len(train_dataset) // hp.train.N, iteration, loss, total_loss / (batch_id + 1))
                print(mesg)
    ...
    # save model
    embedder_net.eval().cpu()
    save_model_filename = "final_epoch_{}".format(e + 1) + "_batch_id_{}".format(batch_id + 1) + ".model"
